...
const convertUtf8ToUint8Array = (input: string) =>
  new TextEncoder("utf-8").encode(input);
const convertUint8ArrayToUtf8 = (input: Uint8Array) =>
...
const getSopsBinPath = () => {
  const sopsPath: string | undefined = vscode.workspace
    .getConfiguration(CONFIG_BASE_SECTION)
    .get(ConfigName.binPath);
  return sopsPath ?? "sops";
};

// TODO wait til vscode provide proper way to get current extension name
const getCurrentExtensionName = (context: vscode.ExtensionContext): string =>
  (context.globalState as any)._id ?? "signageos.signageos-vscode-sops";
...
function getToggleBarText(
  toggleToEncDec: "encrypted" | "decrypted" | "enc/dec" = "enc/dec"
) {
...
toggleStatusBarItem.tooltip =
  "Toggle between original and decrypted file by SOPS";
...
function getSupportedFileFormat(
  languageId: string,
  fileName: string
): IFileFormat | null {
  debug("getSupportedFileFormat", languageId, fileName);
  if (
    ["yaml", "json", "jsonc", "ini", "dotenv", "plaintext", "binary"].includes(
      languageId
    )
  ) {
  ...
  const associations: { [pattern: string]: string } =
  ...
  for (var pattern in associations) {
    ...
    if (
      ...
      associationFileFormat === languageId
    ) {
  ...
  if (!isDecryptedFile(document.uri)) {
    ...
    if (isSopsEncryptedData(fileData)) {
      ...
      await vscode.window.withProgress(progressOptions, async (progress) => {
        progress.report({
          message: `Decrypting "${document.fileName}" SOPS file`,
        });
        await ensureOpenDecryptedFile(
          document.uri,
          document.languageId as IFileFormat
        );
      });
    }
  }
...
function isSopsEncryptedData(fileData: ParsedObject) {
  return (
    ...
    ((typeof fileData.sops === "object" &&
      typeof fileData.sops.version === "string") ||
...
async function handleSaveFile(
  ...
  if (encryptedUri) {
    ...
    await vscode.window.withProgress(progressOptions, async (progress) => {
      progress.report({
        message: `Encrypting "${encryptedUri.path}" SOPS file`,
      });
    ...
    vscode.workspace
      .getConfiguration(CONFIG_BASE_SECTION)
      .get(ConfigName.creationEnabled)
  ) {
    await vscode.window.withProgress(progressOptions, async (progress) => {
      progress.report({
        message: `Trying encrypting new "${decryptedUri.path}" SOPS file`,
      });
...
async function overrideEncryptedFile(
  decryptedUri: vscode.Uri,
  encryptedUri: vscode.Uri,
  fileFormat: IFileFormat
) {
  const originalFileContent = await getDecryptedFileContent(
    encryptedUri,
    fileFormat
  );
  ...
  debug("Content checksums", {
    encryptedContentChecksum,
    decryptedContentChecksum,
  });
...
async function tryCreateEncryptedFile(
  decryptedUri: vscode.Uri,
  fileFormat: IFileFormat
) {
  try {
    const encryptedContent = await getNewEncryptedFileContent(
      decryptedUri,
      fileFormat
    );
    ...
    await vscode.workspace.fs.writeFile(
      encryptedUri,
      convertUtf8ToUint8Array(encryptedContent)
    );
  } catch (error: unknown) {
...
function isNoMatchingRulesError(error: unknown) {
  return (
    ...
    error?.message?.includes("no matching creation rules found")
  );
}
...
function getParser(
  fileFormat: IFileFormat
): (encoded: string) => ParsedObject | ParsedObject[] {
...
async function ensureOpenDecryptedFile(
  encryptedUri: vscode.Uri,
  fileFormat: IFileFormat
) {
  ...
  try {
    ...
    if (
      ...
      vscode.window.showWarningMessage(
        `File seems to be binary and cannot be opened as text: ${uri.path}`
      );
      return;
    }
    throw error;
...
async function decryptFileToFile(
  encryptedUri: vscode.Uri,
  decryptedUri: vscode.Uri,
  fileFormat: IFileFormat
) {
  const decryptedContent = await getDecryptedFileContent(
    encryptedUri,
    fileFormat
  );
  await vscode.workspace.fs.writeFile(
    decryptedUri,
    convertUtf8ToUint8Array(decryptedContent)
  );
}

async function encryptFileToFile(
  decryptedUri: vscode.Uri,
  encryptedUri: vscode.Uri,
  fileFormat: IFileFormat
) {
  const encryptedContent = await getEncryptedFileContent(
    decryptedUri,
    encryptedUri,
    fileFormat
  );
...
async function getDecryptedFileContent(
  uri: vscode.Uri,
  fileFormat: IFileFormat
) {
  const encryptedContent = await getFileContent(uri);
  const tmpEncryptedFilePath = path.join(
    os.tmpdir(),
    await getChecksum(Math.random().toString())
  );
  try {
    ...
    if (!decryptedContent) {
      throw new Error(
        `Could not decrypt file: ${
          uri.path
        }, ${decryptProcess.stderr.toString()}`
      );
    }
...
async function getEncryptedFileContent(
  ...
  let tmpFakeDecryptedEditorPath = path.join(
    os.tmpdir(),
    await getChecksum(Math.random().toString())
  );
  ...
  try {
    await fs.writeFile(tmpFakeDecryptedEditorPath, fakeDecryptedEditor, {
      mode: 0o755,
    });
    await fs.writeFile(tmpDecryptedFilePath, decryptedContent, { mode: 0o600 });
    await fs.writeFile(tmpEncryptedFilePath, originalEncryptedContent, {
      mode: 0o600,
    });
    ...
    const encryptProcess = child_process.spawnSync(sopsBin, cmds, {
      ...spawnOptions,
      env: {
        ...process.env,
        ...envs,
    ...
    const encryptedContent = (
      await fs.readFile(tmpEncryptedFilePath)
    ).toString();
    ...
    if (!encryptedContent) {
      throw new Error(
        `Could not encrypt file: ${
          uri.path
        }, ${encryptProcess.stderr.toString()}`
      );
    }
...
async function getNewEncryptedFileContent(
  decryptedUri: vscode.Uri,
  fileFormat: IFileFormat
) {
  const tmpDirectoryPath = path.join(
    os.tmpdir(),
    await getChecksum(Math.random().toString())
  );
  ...
  try {
    ...
    const sopsConfigUri = await findSopsConfigRecursive(
      decryptedUri.with({ path: path.dirname(decryptedUri.path) })
    );

    let sopsConfigArgs: string[] = [];
    let decryptedRelativePathToSopsConfig: string;
    if (sopsConfigUri) {
      decryptedRelativePathToSopsConfig = path.relative(
        path.dirname(sopsConfigUri.path),
        decryptedUri.path
      );
      const sopsConfigContent = await getFileContent(sopsConfigUri);
      debug("SOP config content", sopsConfigContent);
      const tmpSopsConfigPath = path.join(
        tmpDirectoryPath,
        path.basename(sopsConfigUri.path)
      );
      ...
      await fs.ensureDir(
        path.join(
          tmpDirectoryPath,
          path.dirname(decryptedRelativePathToSopsConfig)
        )
      );
    } else {
    ...
    debug(
      "Decrypted relative path to config",
      decryptedRelativePathToSopsConfig
    );
...
async function findSopsConfigRecursive(
  dirUri: vscode.Uri
  ...
  if (await fileExists(possibleSopsConfigUri)) {
    ...
    path.dirname(dirUri.path) !== "." &&
    path.dirname(dirUri.path) !== "/"
  ) {
    return await findSopsConfigRecursive(
      dirUri.with({ path: path.dirname(dirUri.path) })
    );
  } else {
    return undefined;
  }
...
async function getSopsGeneralOptions(fileUriToEncryptOrDecrypt: vscode.Uri) {
  const defaultAwsProfile: string | undefined = vscode.workspace
    .getConfiguration(CONFIG_BASE_SECTION)
    .get(ConfigName.defaultAwsProfile);
  const defaultGcpCredentialsPath: string | undefined = vscode.workspace
    .getConfiguration(CONFIG_BASE_SECTION)
    .get(ConfigName.defaultGcpCredentialsPath);
  const defaultAgeKeyFile: string | undefined = vscode.workspace
    .getConfiguration(CONFIG_BASE_SECTION)
    .get(ConfigName.defaultAgeKeyFile);
  const ignoreMac: boolean | undefined = vscode.workspace
    .getConfiguration(CONFIG_BASE_SECTION)
    .get(ConfigName.ignoreMac);
  debug("config", {
    defaultAwsProfile,
    defaultGcpCredentialsPath,
    defaultAgeKeyFile,
  });
  ...
  if (gcpCredentialsPath) {
    if (
      !path.isAbsolute(gcpCredentialsPath) &&
      vscode.workspace.workspaceFolders
    ) {
      for (const workspaceFolder of vscode.workspace.workspaceFolders) {
        const gcpCredentialsAbsPath = path.join(
          workspaceFolder.uri.path,
          gcpCredentialsPath
        );
        const gcpCredentialsUri = workspaceFolder.uri.with({
          path: gcpCredentialsAbsPath,
        });
  ...
  if (ageKeyFile) {
    if (!path.isAbsolute(ageKeyFile) && vscode.workspace.workspaceFolders) {
      for (const workspaceFolder of vscode.workspace.workspaceFolders) {
        const ageKeyFileAbsPath = path.join(
          workspaceFolder.uri.path,
          ageKeyFile
        );
        const ageKeyFileUri = workspaceFolder.uri.with({
          path: ageKeyFileAbsPath,
        });
  ...
  let sopsConfigUri = await findSopsConfigRecursive(
    fileUriToEncryptOrDecrypt.with({
      path: path.dirname(fileUriToEncryptOrDecrypt.path),
    })
  );
  if (sopsConfigUri) {
  ...
  if (vscode.workspace.workspaceFolders) {
    if (typeof rcPath === "string") {
      for (const rootPath of vscode.workspace.workspaceFolders) {
        if (rcPath.charAt(0) === "/") {
          ...
          possibleRCUris.push(
            rootPath.uri.with({ path: path.join(rootPath.uri.path, rcPath) })
          );
        }
      }
    }
    if (!rcPath) {
      for (const rootPath of vscode.workspace.workspaceFolders) {
        possibleRCUris.push(
          rootPath.uri.with({
            path: path.join(rootPath.uri.path, DEFAULT_RUN_CONTROL_FILENAME),
          })
        );
      }
    }
...
function getDecryptedFileUri(encryptedUri: vscode.Uri): vscode.Uri {
  ...
  const decryptedFilePath = path.join(
    path.dirname(encryptedUri.path),
    decryptedFileName
  );
...
function getEncryptedFileUri(decryptedUri: vscode.Uri): vscode.Uri | null {
  return isDecryptedFile(decryptedUri)
    ? decryptedUri.with({
        path: normalizeCrossPlatformPath(
          path.join(
...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function syncStatusBar() {
    ...
    if (lastActiveEditor) {
      if (isDecryptedFile(lastActiveEditor.document.uri)) {
        ...
        debug(
          "showing status bar for decrypted file",
          lastActiveEditor.document.uri.path
        );
        toggleStatusBarItem.show();
        return;
      }
      const decryptedFileUri = getDecryptedFileUri(
        lastActiveEditor.document.uri
      );
      if (decryptedFileUri && (await fileExists(decryptedFileUri))) {
        toggleStatusBarItem.text = getToggleBarText("decrypted");
        debug(
          "showing status bar for encrypted file",
          lastActiveEditor.document.uri.path
        );
        toggleStatusBarItem.show();
        return;
  ...
  const toggleOriginalFile = async () => {
    ...
    if (lastActiveEditor) {
      ...
      const encryptedFileUri = getEncryptedFileUri(
        lastActiveEditor.document.uri
      );
    ...
    if (editor) {
        ...
        for (const decryptedFileUri of decryptedFileUris) {
          try {
            ...
            vscode.window.showErrorMessage(
              `Could not delete decrypted SOPS file ${
      ...
      try {
        if (!document.isUntitled) {
          const fileFormat = getSupportedFileFormat(
            document.languageId,
            document.fileName
          );
          if (
            fileFormat &&
            !(await fileExists(getDecryptedFileUri(document.uri)))
          ) {
            await handleFile(document, fileFormat);
          }
        }
        ...
        if (!(error instanceof ParseError)) {
          vscode.window.showErrorMessage(
            `Could not decrypt SOPS file ${document.fileName}: ${
              error instanceof Error ? error.message : error
            }`
          );
        }
  ...
  const onTextDocumentSaved = async (document: vscode.TextDocument) => {
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Could not encrypt SOPS file ${document.fileName}: ${
          error instanceof Error ? error.message : error
        }`
      );
    }
  ...
  const createSetBeta = (enableBeta: boolean) => () => {
    vscode.workspace
      .getConfiguration(CONFIG_BASE_SECTION)
      .update(ConfigName.beta, enableBeta, vscode.ConfigurationTarget.Global);
  };
    ...
    if (isEnabled(context)) {
      ...
      while ((activeDisposable = activeDisposables.pop())) {
        ...
        context.subscriptions.splice(
          context.subscriptions.indexOf(activeDisposable),
          1
        );
      }
    }
  ...
  const configurationChangesDisposable =
